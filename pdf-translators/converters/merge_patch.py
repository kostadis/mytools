...
Workflow for missing/bad pages:
    ...
    python3 converters/pdf_to_5etools_v2.py INPUT.pdf --pages 14-22 --pages-per-chunk 2 \\
        --out patch.json --debug-dir debug-patch/

    # 2. Preview what will be inserted and where:
    python3 converters/merge_patch.py adventure.json patch.json --at 13 --dry-run

    # 3. Merge (creates a .bak backup first):
    python3 converters/merge_patch.py adventure.json patch.json --at 13

The --at index is the data[] position BEFORE which the new sections are
inserted.  Run `python3 converters/merge_patch.py adventure.json --list` to print the
...
from lib.fix_adventure_json import assign_ids, reset_ids
