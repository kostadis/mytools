# PDF → 5etools Converters (v1 — superseded)

**This documents the retired v1 heuristic converters, which have been replaced by [`pdf_to_5etools_v2.py`](README.md).** Most of the scripts below no longer exist in this repo — `pdf_to_5etools.py` and `pdf_to_5etools_ocr.py` survive only inside `files-8.zip`; `pdf_to_5etools_1e.py` and `test_pdf_to_5etools.py` appear to be lost entirely. Kept for historical reference only.
