...
Unlike v1 (three separate heuristic scripts — see [README.md](README.md)), v2 is a **single unified converter** that auto-routes each PDF to the right extraction path:
...
The prior-generation heuristic converters (`pdf_to_5etools.py`, `pdf_to_5etools_ocr.py`, `pdf_to_5etools_1e.py`, plus their TOC variants and repair scripts) are preserved at tag `v1.0` and documented in [README.md](README.md). To resurrect any v1 file:

```bash
git checkout v1.0 -- pdf-translators/<filename>
```
