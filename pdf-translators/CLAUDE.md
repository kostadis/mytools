...
**Common flags:** `--dry-run` (estimate cost, no API calls), `--batch` (50% cheaper via Batch API, async, Anthropic-only), `--concurrency N` (send N chunks at once on the streaming path), `--output-mode server` (two-file permanent install), `--extract-monsters`/`--monsters-only` (bestiary extraction, see below), `--debug-dir DIR` (save raw chunk I/O), `--verbose`. See `cli_args.py` for the full shared argument set.
