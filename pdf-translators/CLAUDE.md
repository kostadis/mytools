...
Converts tabletop RPG PDFs (primarily D&D/AD&D sourcebooks and modules) into [5etools](https://5e.tools) homebrew JSON format. A single unified converter (`converters/pdf_to_5etools_v2.py`) handles all PDF types — digital, scanned, OCR'd 1e/2e modules — by routing bookmarked digital PDFs to a fast PyMuPDF path and everything else to [Marker](https://github.com/VikParuchuri/marker) for ML-based layout and heading extraction. Wrapped by a Flask web UI.

Prior-generation v1 heuristic converters (six scripts) are documented in `README_v1.md` / `ARCHITECTURE_V1.md` — no `v1.0` git tag actually exists. See the "v1 / v2 history" section below.
...
├── batch/          batch drivers + their working data (batch_convert, batch_marker,
│                   batch_mistral_ocr, ocr_three, dmsguild-state.db, dmsguild-logs/, ...)
├── editors/        Flask UIs (app, toc_editor, toc_fixer, monster_editor,
│                   adventure_editor, markdown_editor, editor_server, probe_editor)
├── tests/          all test_*.py (pytest)
├── scripts/        shell entry points (convert_all_ddex.sh, convert_ddex3.sh,
│                   convert_dmsguild.sh, run_convert.sh, start_editor.sh)
├── frontend/       Vite/Vue SPA served by editors/editor_server.py
├── conftest.py     empty — puts the project root on sys.path for pytest
└── CLAUDE.md, README.md, ARCHITECTURE.md, BATCH.md
```

Every entry-point script bootstraps the project root onto `sys.path`
(`sys.path.insert(0, str(Path(__file__).resolve().parent.parent))`) before its
sibling imports, since these are run directly (`python3 converters/foo.py`),
not via `python -m`. Cross-subpackage imports are absolute and rooted at the
project root (e.g. `from lib.pdf_utils import TocNode`). Pure library modules
that are only ever imported (never run standalone) don't carry their own
bootstrap — whatever imported them already added it.

## Running tests

```bash
pytest tests/test_adventure_model.py -v       # adventure data model tests
pytest tests/test_adventure_editor.py -v      # adventure editor tests
pytest tests/test_validate_adventure.py -v    # JSON structure validator (includes all official adventures)
```
...
```bash
pytest tests/test_adventure_model.py -v -k "test_function_name"
pytest tests/test_adventure_editor.py -v -k "test_function_name"
pytest tests/test_validate_adventure.py -v -k "test_function_name"
```

## Running the web UI

```bash
python3 editors/app.py          # serves at http://localhost:5100
...
**`claude-code` provider (subscription):** routes every chunk through `claude -p --output-format json` via the `lib/claudecodelib.py` transport, so conversions draw down your Claude Code subscription rather than API credits. Requirements and constraints:
...
**Common flags:** `--dry-run` (estimate cost, no API calls), `--batch` (50% cheaper via Batch API, async, Anthropic-only), `--concurrency N` (send N chunks at once on the streaming path), `--reuse-responses` (resume a crashed streaming run from saved responses, see below), `--output-mode server` (two-file permanent install), `--extract-monsters`/`--monsters-only` (bestiary extraction, see below), `--debug-dir DIR` (save raw chunk I/O), `--verbose`. See `lib/cli_args.py` for the full shared argument set.
...
A CUDA GPU is strongly recommended (4080 or similar: ~5 s/page; CPU: 10–30 s/page). The venv is gitignored. `marker-env/` and `frontend/` both stay at the project root regardless of which subpackage references them (`converters/pdf_to_5etools_v2.py`'s `MARKER_ENV` and `editors/editor_server.py`'s `DIST` resolve up to the root explicitly).

## Architecture

### Shared CLI layer — `lib/cli_args.py`

`converters/pdf_to_5etools_v2.py` imports from `lib/cli_args.py` for its argparse setup:
...
**When adding or changing any shared CLI argument, edit `lib/cli_args.py` only.**

### Provider/transport layer — `lib/llm_backend.py`

`lib/claude_api.py` no longer touches the Anthropic SDK directly. It calls a small provider-agnostic `Backend` object that wraps one of three transport libraries:
- **`claudelib`** (editable install at the mytools repo root, shared with rpg-lib/CampaignGenerator — distinct from this project's own `lib/` subpackage) — the Anthropic Messages API.
...
To enrich the transport (e.g. expose more of the response), add a richer call to **all** transport libs (`call_api_full` is the precedent), then surface it through `Backend`. The libs return only a normalised `stop_reason`; provider-specific strings never leak into `lib/claude_api.py`.

### Shared API layer — `lib/claude_api.py`

All converters delegate LLM calls to `lib/claude_api.py`, which talks to a `Backend` (see `lib/llm_backend.py` above) rather than the Anthropic SDK directly. It owns:
...
Each converter's `call_claude` is a thin wrapper that passes its own `SYSTEM_PROMPT` and handles any converter-specific preprocessing (1e: `_CHUNK_PREFIX + _neutralize_triggers + _sanitize_text`) or error handling (1e: `BadRequestError` → `None`). Future fixes to retry/parse/prompt logic go in `lib/claude_api.py` only.

`lib.claude_api` is imported at module top-level (before `SYSTEM_PROMPT` is defined) so that `_api.COMMON_TAG_RULES` is available when the f-string prompt is constructed.

### PDF bookmark / TOC extraction — `lib/pdf_utils.py`

`lib/pdf_utils.py` is a shared library (depends on PyMuPDF, kept separate from `lib/claude_api.py` which has no PDF dependency) that owns:
...
`converters/pdf_to_5etools_v2.py` imports `TocNode`, `get_toc_tree`, `parse_toc_tree`, and `extract_pdf_toc` for both the fast and Marker paths.

### v2 converter pipeline — `converters/pdf_to_5etools_v2.py`
...
**System prompt** (in `converters.pdf_to_5etools_v2.SYSTEM_PROMPT`) is deliberately slim: markdown headings are declared authoritative, so Claude renders prose inside pre-built structure rather than inferring structure from font heuristics. `{_api.COMMON_TAG_RULES}` and `{_api.COMMON_NESTING_RULES}` are still injected.

### Typed data model — `lib/adventure_model.py`
...
**Imports from sibling modules:** `lib.validate_adventure.{VALID_ENTRY_TYPES, KNOWN_TAGS, TAG_RE}`.

**Tests:** `pytest tests/test_adventure_model.py -v` (~90 tests covering entry construction, validation modes, serialization round-trips, document parsing, contents/data alignment, ID assignment, TOC building, and integration against official adventure files).
...
### `editors/toc_editor.py` — TOC editor UI
...
```bash
python3 editors/toc_editor.py [file.json] [--port N]
```

### `editors/toc_fixer.py` — heuristic TOC & nesting repair
...
```bash
python3 editors/toc_fixer.py [file.json] [--pdf file.pdf] [--port N]
# http://localhost:5102
```
...
**Imports from sibling modules:** `lib.fix_adventure_json.{assign_ids, reset_ids, build_toc}`, `editors.toc_editor.list_json_files`, `lib.pdf_utils._decode_pdf_string`.

### `lib/fix_adventure_json.py` — chapter-index normalizer

Post-processes a converter-generated adventure JSON to fix chapter-index mismatches. Non-section top-level entries in `data[]` cause `contents[i]` and `data[i]` to diverge, breaking sidebar navigation. Also used as a library by `editors/toc_fixer.py`.
...
```bash
python3 converters/patch_5e_chapters.py source_1e.json target_5e.json --chapters 16,19-20 [--model MODEL]
```

### `converters/merge_patch.py` — patch incomplete conversions
...
```bash
python3 converters/merge_patch.py adventure.json --list                    # show sections with indices
python3 converters/merge_patch.py adventure.json patch.json --at N [--dry-run]
```

Creates a `.bak` backup, re-sequences IDs after merge, and reports TOC/data alignment mismatches.

### `editors/app.py` — Flask web UI
...
```bash
python3 converters/convert_1e_to_5e.py input.json output.json [--chapters A-B] [--dry-run] [--model MODEL]
```

Default model is `claude-sonnet-4-6`. Contains hardcoded T1-4 zone/level mappings — adapt `ZONES` dict for other modules.

### `lib/validate_tags.py` — post-conversion tag checker
...
```bash
python3 lib/validate_tags.py adventure.json           # report unknown tags
python3 lib/validate_tags.py adventure.json --fix     # replace in-place with plain text
```
...
```bash
python3 converters/extract_monsters.py adventure.json                    # extract all stat blocks
python3 converters/extract_monsters.py adventure.json --dry-run          # list found blocks, no API calls
...
Flask app (port 5103) for interactive stat block discovery and extraction from a parsed adventure JSON. Imports discovery logic from `converters/extract_monsters.py` and API calls from `lib/claude_api.py`.

```bash
python3 editors/monster_editor.py [file.json] [--port N]
# http://localhost:5103
```
...
**Imports from sibling modules:** `converters.extract_monsters.{_has_ac_table, statblock_to_text, SYSTEM_PROMPT}`, `lib.claude_api.call_claude`, `editors.toc_editor.list_json_files`.

### `editors/editor_server.py` + `frontend/` — unified Markdown/Adventure editor (Vite/Vue SPA)

Single Flask app (port 5107) serving a single Vue 3 SPA that hosts **two** editors behind a left sidebar: **Markdown Editor** (heading-tree cleanup for the Markdown that feeds `converters/pdf_to_5etools_v2.py --from-markdown`, typically OCR output from `converters/extract_markdown.py`) and **Adventure Editor** (visual block-tree editor for 5etools adventure/book JSON, with a live CSS-approximated preview). Switching editors in the sidebar swaps the central pane client-side — no page reload — and a file loaded in one editor stays loaded when you switch away and back.

```bash
python3 editors/editor_server.py [file.md|adventure.json] [--port N]
# http://127.0.0.1:5107
# or: ./scripts/start_editor.sh [file]
```
...
**Testing split:** pure tree/parse logic is tested in Vitest (`frontend/src/lib/*.test.ts`) against the real TS, not a hand-copied Python reimplementation — `tests/test_adventure_editor.py` keeps only genuine backend coverage (load/save/ID-TOC-rebuild, undo-log persistence, the `/api/adv/*` routes).

**Build step (required — the bundle is gitignored):**
```bash
cd frontend && npm install && npm run build     # produces frontend/dist/
python3 editors/editor_server.py [file] [--port N]      # http://127.0.0.1:5107
```
...
**Diagnostic:** `editors/probe_editor.py` is a stdlib CLI that probes any of these Flask UIs and reports a per-phase timing breakdown (TCP connect / time-to-first-byte / body read) so "slow" can be localized to network vs. server vs. browser. `python3 editors/probe_editor.py --port 5107`.

### `lib/validate_adventure.py` — adventure JSON structural validator
...
```bash
python3 lib/validate_adventure.py adventure.json                              # validate one file
python3 lib/validate_adventure.py *.json                                      # validate multiple
python3 lib/validate_adventure.py --official-dir ../data/adventure/           # validate official files
```
...
**As a library:** `from lib.validate_adventure import validate; result = validate(json_data)` returns a `ValidationResult` with `.errors`, `.warnings`, `.ok`.

**Tests:** `pytest tests/test_validate_adventure.py -v` (44 tests including integration against all 98 official adventure files).

## Key 1e stat conversion formulas

Used by `converters/convert_1e_to_5e.py` (post-conversion mechanical rewrite). v2 keeps stat lines verbatim as italic strings; mechanical conversion is a separate pass.

```
5e AC = 19 − 1e_AC          (descending → ascending)
attack bonus = 20 − THAC0
speed (ft) = MV_inches × 5
CR = table lookup from HD (see hd_to_cr() in converters/convert_1e_to_5e.py)
```

## v1 / v2 history

Prior-generation v1 heuristic converters, documented in `README_v1.md` / `ARCHITECTURE_V1.md`:
...
**No `v1.0` git tag exists in this repo**, despite this section previously claiming one. `files-8.zip` (containing `pdf_to_5etools.py` and `pdf_to_5etools_ocr.py`) and `v1/pdf_utils_old.py` are the only surviving v1 code; `pdf_to_5etools_1e.py`, the TOC/repair-script variants, and the trigger-substitution infrastructure appear to be lost entirely.
...
- **TOC / structured TOC live in `lib/pdf_utils.py`** (`extract_pdf_toc`, `TocNode`, `parse_toc_tree`, `get_toc_tree`, `_decode_pdf_string`).
- **Adventure data model lives in `lib/adventure_model.py`** — typed dataclass model for constructing and validating entries. Used by v2 for assembly and by `claude_api.validate_entries()` for validation retry. Tag/entry-type constants come from `lib/validate_adventure.py`.
- **`{@tag}` validation** — run `python3 lib/validate_tags.py adventure.json` after conversion to catch unknown tags (which cause blank pages in 5etools). Use `--fix` to replace them with plain text in-place.
...
- **Never put `pk` (JSON path keys) in HTML strings or `onclick` attributes.** Path keys like `[0,"entries",2]` contain double quotes that break HTML attribute parsing. Always use `addEventListener` with closures instead. Use CSS class names (`.btn-done`, `.btn-cancel`, `.btn-add-child`) on the HTML elements, then attach handlers after setting `innerHTML`. Applies to the remaining inline-HTML/vanilla-JS apps (`editors/toc_editor.py`, `editors/toc_fixer.py`, `editors/monster_editor.py`) — the Vue-based editors (`editors/editor_server.py` + `frontend/`) can't have this bug at all, since Vue's `@click` bindings never serialize JS values into HTML attribute strings.

## Refactoring rule

When you find logic (parsing, prompt fragments, retry handling, tag rules, etc.) that could live in a shared module (`lib/claude_api.py`, `lib/pdf_utils.py`, `lib/adventure_model.py`) instead of in `converters/pdf_to_5etools_v2.py`, **ask the user whether to hoist it before proceeding**. The goal is that any fix or enhancement to shared behaviour is made in exactly one place.
