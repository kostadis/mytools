...
# Per-room housekeeping headings that adventure modules repeat under every
# keyed room. Historically we suppressed these by capping ``max_level`` at 3,
# which worked for keyed-room PDFs but hid L4 content that was actually
# top-level (e.g. DDEX modules nesting "Appendix: Monster/NPC Statistics"
# and every individual stat-block name at L4). Blacklisting by title keeps
# the noise out regardless of depth, letting appendices through.
#
# Match rule: case-insensitive comparison against the stripped title, with
# any trailing ``:`` / ``.`` / whitespace removed. Entries are bare phrases
# so "Treasure" matches "Treasure" and "Treasure:" but NOT
# "Treasure Awards" or "Appendix: Treasure Trove".
_NOISE_BOOKMARK_TITLES: frozenset[str] = frozenset({
    "treasure",
    "xp award",
    "xp awards",
    "experience",           # when nested as a per-encounter award subsection
    "experience points",
    "development",
    "developments",
    "general features",
    ...
    "tactics",
    "reward",
    "rewards",              # per-encounter reward blurbs, not chapter-level
})

# Per-room secret-mission headings vary by faction ("Secret Mission:
# Emerald Enclave", "Secret Mission: Order of the Gauntlet", …) so they
# need a prefix match rather than exact equality.
_NOISE_BOOKMARK_PREFIXES: tuple[str, ...] = (
    "secret mission",
)


def is_noise_bookmark(title: str) -> bool:
    """Return True for per-room housekeeping bookmarks.

    These titles repeat under every keyed room in an adventure module and
    don't correspond to real chapter structure. Filtering them at tree-
    build time prevents them from polluting chunk boundaries or inflating
    the ``headers[]`` hint sent to Claude. See ``_NOISE_BOOKMARK_TITLES``
    for the full list.
    """
    normalised = title.strip().rstrip(":.").strip().lower()
    if not normalised:
        return False
    if normalised in _NOISE_BOOKMARK_TITLES:
        return True
    for prefix in _NOISE_BOOKMARK_PREFIXES:
...
def parse_toc_tree(raw_toc: list[list], total_pages: int,
                   max_level: int = 99,
                   *,
                   skip_anchor_bookmarks: bool = True,
                   skip_noise_bookmarks: bool = True) -> list[TocNode]:
