#!/usr/bin/env python3
"""Parse Barbarian HTML using proper HTML parsing (no regex)."""

import json
import re
from html.parser import HTMLParser
from class_extractor.utils import clean_description


class BarbarianHTMLParser(HTMLParser):
    """HTML parser to extract Barbarian class data."""
    
    def __init__(self):
        super().__init__()
        self.data = {
            "name": "Barbarian",
            "edition": "2024",
            "ui": {
                "skillPicks": 2,
                "hasExpertise": False,
                "subclassDesc": "Choose your Primal Path at level 3.",
                "featDesc": "General Feats fill your ASI slots.",
                "backgroundNote": "In 2024 D&D, your Background sets ability scores (+2/+1) and grants an Origin Feat.",
                "multiclassNote": "Barbarians multiclass well with classes that benefit from high Strength and Constitution.",
                "weaponNote": "Barbarians use Strength for attack and damage rolls with melee weapons.",
                "armorNote": "Barbarians are proficient in light and medium armor, plus shields."
            },
            "subclasses": [],
            "species": [],
            "abilityNotes": [],
            "skills": [],
            "backgrounds": [],
            "feats": [],
            "epicBoons": [],
            "classFeatures": [],
            "weapons": [],
            "armor": []
        }
        
        # State tracking
        self.in_subclass_list = False
        self.in_subclass_li = False
        self.in_p = False
        self.current_text = []
        self.in_script = False
        self.in_style = False
    
    def handle_starttag(self, tag, attrs):
        attrs_dict = dict(attrs)
        class_attr = attrs_dict.get('class', '')
        
        # Skip script and style tags
        if tag == 'script':
            self.in_script = True
            return
        if tag == 'style':
            self.in_style = True
            return
        
        # Check for subclass list (wp-block-list ul)
        if tag == 'ul' and 'wp-block-list' in str(class_attr):
            self.in_subclass_list = True
        
        if tag == 'li' and self.in_subclass_list:
            self.in_subclass_li = True
            self.current_text = []
        
        # Track paragraph tags
        if tag == 'p':
            self.in_p = True
            self.current_text = []
    
    def handle_endtag(self, tag):
        if tag == 'script':
            self.in_script = False
            return
        if tag == 'style':
            self.in_style = False
            return
        
        if tag == 'ul':
            self.in_subclass_list = False
        
        if tag == 'li' and self.in_subclass_li:
            self._process_subclass_li()
            self.in_subclass_li = False
        
        if tag == 'p' and self.in_p:
            text = ''.join(self.current_text).strip()
            if text:
                self._process_paragraph(text)
            self.in_p = False
            self.current_text = []
    
    def handle_data(self, data):
        if self.in_script or self.in_style:
            return
        
        if self.in_subclass_li or self.in_p:
            self.current_text.append(data)
    
    def _process_subclass_li(self):
        """Process extracted subclass from li element."""
        text = ''.join(self.current_text).strip()
        
        # Check if this is a "Path of the X" entry
        if 'Path of the' in text and ':' in text:
            # Split on first colon
            colon_idx = text.index(':')
            full_name = text[:colon_idx].strip()
            desc = text[colon_idx + 1:].strip()
            
            # Extract subclass name (remove "Path of the")
            if full_name.startswith('Path of the '):
                name = full_name[12:].strip()
            else:
                name = full_name
            
            # Clean description (remove any remaining HTML artifacts and normalize whitespace)
            desc = re.sub(r'\s+', ' ', desc)
            desc = clean_description(desc, max_length=250)
            
            # Determine rating
            rating = self._get_rating(desc)
            
            self.data["subclasses"].append({
                "name": name,
                "r": rating,
                "d": desc
            })
    
    def _process_paragraph(self, text: str):
        """Process a paragraph that might contain class features or other data."""
        # Check for numbered class features: "N. FeatureName: Description"
        # Look for pattern like "1. Weapon Mastery:" or "2. Danger Sense:"
        match = re.match(r'^(\d+)\.\s+([A-Z][a-zA-Z\s]+):\s*(.+)$', text, re.DOTALL)
        
        if match:
            level = int(match.group(1))
            name = match.group(2).strip()
            desc = match.group(3).strip()
            
            # Clean description
            desc = re.sub(r'\s+', ' ', desc).strip()
            desc = clean_description(desc, max_length=300)
            
            # Determine rating from text
            rating = self._get_rating(desc)
            
            # Avoid duplicates
            if not any(feat['n'] == name and feat['lv'] == level for feat in self.data["classFeatures"]):
                self.data["classFeatures"].append({
                    "lv": level,
                    "n": name,
                    "r": rating,
                    "d": desc
                })
            return
        
        # Check for ability score patterns: "Str: Description" or "Strength: Description"
        ab_match = re.match(r'^(Str|Dex|Con|Int|Wis|Cha|Strength|Dexterity|Constitution|Intelligence|Wisdom|Charisma):\s*(.+)$', text)
        if ab_match:
            stat = ab_match.group(1)
            # Normalize stat names
            stat_map = {
                'Strength': 'Str', 'Dexterity': 'Dex', 'Constitution': 'Con',
                'Intelligence': 'Int', 'Wisdom': 'Wis', 'Charisma': 'Cha'
            }
            stat = stat_map.get(stat, stat)
            
            desc = ab_match.group(2).strip()
            desc = clean_description(desc, max_length=200)
            
            rating = self._get_rating(desc)
            
            # Avoid duplicates
            if not any(note['s'] == stat and note['n'] == desc for note in self.data["abilityNotes"]):
                self.data["abilityNotes"].append({
                    "s": stat,
                    "r": rating,
                    "n": desc
                })
            return
        
        # Check for skill patterns: "SkillName (Abbrev): Description"
        skill_match = re.match(r'^([A-Z][a-z]+)\s+\([A-Z][a-z]{2}\):\s*(.+)$', text)
        if skill_match:
            name = skill_match.group(1).strip()
            desc = skill_match.group(2).strip()
            desc = clean_description(desc, max_length=200)
            
            rating = self._get_rating(desc)
            
            # Avoid duplicates
            if not any(skill['n'] == name for skill in self.data["skills"]):
                self.data["skills"].append({
                    "n": name,
                    "r": rating,
                    "d": desc
                })
            return
    
    def _get_rating(self, desc: str) -> str:
        """Determine rating based on description text."""
        desc_lower = desc.lower()
        if any(word in desc_lower for word in ['fantastic', 'essential', 'amazing', 'best', 'crucial']):
            return 'blue'
        elif any(word in desc_lower for word in ['good', 'great', 'useful', 'important', 'reliable']):
            return 'green'
        elif any(word in desc_lower for word in ['bad', 'useless', 'terrible', 'skip', 'dump']):
            return 'red'
        return 'orange'
    
    def extract(self, html_content: str):
        """Parse HTML and return extracted data."""
        self.feed(html_content)
        return self.data


def extract_barbarian_from_html(html_file: str, output_file: str | None = None):
    """Extract Barbarian class data from HTML file using proper HTML parsing."""
    
    with open(html_file) as f:
        html = f.read()
    
    parser = BarbarianHTMLParser()
    data = parser.extract(html)
    
    # Print summary
    print(f"=== Barbarian 2024 ===")
    print(f"Subclasses: {len(data['subclasses'])}")
    for sub in data['subclasses']:
        print(f"  - {sub['name']} ({sub['r']})")
    
    print(f"Class Features: {len(data['classFeatures'])}")
    for feat in data['classFeatures']:
        print(f"  Lv{feat['lv']}: {feat['n']} ({feat['r']})")
    
    print(f"Ability Notes: {len(data['abilityNotes'])}")
    for note in data['abilityNotes']:
        print(f"  {note['s']}: ({note['r']})")
    
    print(f"Skills: {len(data['skills'])}")
    for skill in data['skills']:
        print(f"  {skill['n']} ({skill['r']})")
    
    # Write output
    if output_file:
        with open(output_file, 'w') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        print(f"\nWritten to: {output_file}")
    
    return data


if __name__ == "__main__":
    import sys
    
    if len(sys.argv) < 2:
        print("Usage: python parse_barbarian_html.py <input.html> [output.json]")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2] if len(sys.argv) > 2 else 'data/barbarian-2024.json'
    
    extract_barbarian_from_html(input_file, output_file)
