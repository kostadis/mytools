---
name: remove-recap
description: Detect and remove the previous-chapter recap at the start of a recorded session while rescuing new canon, current-chapter bookkeeping, and missed upstream beats. Use when the user asks for /remove-recap [session-dir] or wants a GM-reviewed recap-removal pass that edits only derived layers and never the verbatim scene extractions or VTT.
metadata:
  short-description: Remove an opening previous-chapter recap
---
...
`~/src/mytools/dotfiles/claude/skills/remove-recap/` when changing this skill.

## Codex Compatibility

- Ask the GM questions in chat; do not refer to Claude `AskUserQuestion`.
- Detection and rescue analysis may run without approval. Any recap boundary,
  rescued-content destination, file deletion, or downstream rebuild requires
  the GM's explicit ruling first.
- Use `apply_patch` for approved Markdown edits. When trimming only a prefix in
  an existing smoothed extraction, use the `no-mech` deterministic applier so
  its path and stale-line guards remain active.
- Deleting an approved derived scene is allowed only after resolving the exact
  path and restating the renumbering consequence. Never delete the verbatim
  counterpart.
- Re-narration may invoke a paid or remote backend. Explain its scope and obtain
...
```bash
python ~/.codex/skills/remove-recap/find_recap.py \
...
```bash
python ~/.codex/skills/remove-recap/recap_unique.py \
