...
- Sixteen skills ship **helper Python scripts** next to `SKILL.md` — `vtt-spell-pass` (10), `scrub` (5), `speaker-attribution` (5), `dossier-merge`, `ensemble-type-merge`, `module-inventory`, `registry-cleanup`, `no-mech`, `remove-recap`, `transcript-rebuild` (2 each), and `campaign-chapter-review`, `chapter-enhance`, `chapter-summarise`, `dialogue-edit`, `speaker-attribution-text`, `staged-consistency` (1 each). These are the deterministic halves of the propose/apply split; the SKILL.md invokes them by path. Keep script and prose in sync when changing either.
- Some skill files are shared with the Codex copies in `codex/skills/` and must stay byte-identical; `skill-sync.txt` lists them. After changing one, port it to the other copy and run `./check-skill-sync.sh` (exit 1 on drift).
