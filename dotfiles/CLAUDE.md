...
**Auto-recall memory is NOT in this repo.** `~/.claude/memory/` and `~/.claude/projects/<slug>/memory/` are owned by a separate repo, `~/src/claude-memory` ([github.com/kostadis/claude-memory](https://github.com/kostadis/claude-memory)), which syncs them across all machines via `~/.claude/bin/sync-memory.sh` on a 30-minute cron. Each machine writes only its own `machines/<hostname>/` namespace and union-merges the others, so no two machines clobber each other. `claude/memory/` used to be tracked here as well; that made mytools a second authority, the copies drifted, and the cron wrote foreign machines' files into this working tree through a symlink. It is now gitignored — **do not re-add it.** Memory belongs to `claude-memory`; this repo owns config, skills, and agents.
