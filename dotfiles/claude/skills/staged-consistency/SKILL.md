...
After each scene's fixes, ask: "Continue to next scene, or revisit this one?" Don't auto-advance through all scenes silently. **This is the stage where finding counts routinely justify the artifact alternative** (see "Alternative sign-off" above) instead of a 1x1 walkthrough per scene — offer it once you can see the likely total finding count is large.
