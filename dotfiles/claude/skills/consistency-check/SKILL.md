...
Grep the session transcript around the disputed beat. **Pick the file by what the question needs:** for *attribution* questions prefer a speaker-labelled Zoom `.md` (step 1) over any VTT; for *wording/quote* questions prefer `*.retranscribed.cleaned.vtt`, then `*.cleaned.vtt`.

**Attribution is settled by speaker labels, never by a summary.** When there is no Zoom `.md`, the raw `*.transcript.vtt` carries the real-name labels and `*.transcript.cleaned.vtt` the derived PC-name ones: read *from* the cleaned file, adjudicate *against* the raw one. Build the name map once and hold it (`grep -oP '(?<=^)[A-Z][A-Za-z .\x27-]+(?=:)' <raw>.vtt | sort | uniq -c | sort -rn`, matched by cue count against the cleaned file). ⛔ Never settle "who said or did X" from a recap, from Zoom's summary, or from the two agreeing — see the Ch 65 case in Notes.

**`zoom-summary.md`, if present, is a structural cross-check and nothing more.** It is good at what was in a room, how many options an NPC offered and which beats a scene had, so it catches material the recap *dropped*. It is unreliable at attribution for the same reason the GMAssist extractor is: neither has speaker labels in front of it while it writes prose. ⭐ **Two summaries agreeing on "who" is not corroboration, it is a shared failure mode** — treat that agreement as zero evidence and go to the labels. If `zoom-summary.md` is absent, say so in the summary: the run has lost the dropped-beat check, which prep cannot supply.

Four classes justify the trip:
...
2. **Attribution questions** — who did/said/killed/freed what. These are *precision decisions*: never guess, always adjudicate. And not just kills: who made the pitch, who rolled, who asked the question. Extractors fail here most, and most silently, because the *event* is right and only the name is wrong — so check every named action against the speaker labels, not only the ones the report flagged.
...
4. **Planned vs. resolved.** Extractors write a resolved action as an *intention* when the session ends near it. If the GM confirmed a position or an outcome on tape, the recap must say it happened — the next session's opening state depends on it.
...
**The GM takes back rulings too, not just names.** A retraction is often a half-sentence mid-cue (*"oop, sorry, that's not…"*) that every summariser reads as noise, so grep the transcript around any number the recap asserts. ⛔ **But a retraction is not self-justifying** — check the rule before recording the take-back as correct. A GM mid-combat may be reaching for the wrong edition's text, which is exactly what happened in Ch 65 (Notes).

**Players retcon too, after the fact.** A player can go back and rewrite an earlier turn (*"Can I go back and modify some stuff?"*, a bonus action they forgot), and the GM accepts it without restating the outcome. The tape then says *"Knocked out"* and, a minute later, *"are you dead or alive?" "Alive."* Neither the summary nor a quick read of the tape reconciles the two. On OOTA ch03 (2026-09-25), Thorin's Second Wind retcon at 19:28 undid the elite warrior's knockout, and the "again" in *"I am again knocked out"* four minutes later is Joe counting the knockout the retcon had removed. The card went out saying "two knockouts", was approved, and only then was the retcon found. The GM ruled one. **Before counting knockouts, deaths, hit points or spell slots, grep the minutes after each one for `go back`, `modify`, `retroactive`, `forgot`, `should have`, and `dead or alive`.**
...
**A context file the check calls "authoritative" is not, just because it is session-specific.** The model promotes any document that narrates *this* session — a bible chapter, a prior recap — to an "authoritative tier" of its own and builds findings on it. On OOTA ch02 (2026-09-25, a backfill), the bible split `docs/chapters/chapter_01_exploring_the_prison.md` was passed as prep-substitute corroboration; the report headed its evidence *"the session document this recap summarizes — authoritative tier"*, and **10 of its 19 findings rested on it. The tape overturned all 10** — Shoor *was* in Ilvara's quarters (*"She is here, as is Shoor Vandree"*), the injury *was* a black pudding and two fingers, the spools thread the report called fabricated is on tape. The bible is the GM's later prose rendering: it drops beats, adds NPC names, and changes details. Treat any finding whose only evidence is such a document exactly like a grounding-doc finding below — a lead to take to the tape, never a fix. It recurred on the very next chapter: on OOTA ch03, Stage 0 and Stage 1 both cited the bible split for Sava instead of the playing cards on tape, a goblet instead of the kicked can, telepathic calming, Sarith driven mad by Derendil's death, and Imbros as the Mind Sliver target. Every one was overturned by the tape. Where the bible pointed at a real error (who calmed Sarith, how many heals), the fix still had to come from the tape, because the bible's own version of the detail was wrong too.
...
⛔ **Never write "RAW" without naming the edition.** 5e has two live PHBs and they disagree on damage on a made save, on conditions and on riders. Before accepting or carding any mechanics finding, read the actual entry and cite it as *"PHB 2024, p.304"*, never a bare "which is RAW". If the table's edition is not established, that is the question to put to the GM, not the mechanics. The 5etools MCP is often down; the source data is on disk and settles it in one read: `~/src/5etools-src/data/spells/spells-phb.json` (2014) and `spells-xphb.json` (2024). The tape often names the edition too — a player quoting the rules verbatim is evidence of which book the table is using.
...
- **The OOTA Ch 65 run (2026-08-27) is the case behind the attribution, retraction and edition rules in steps 4.7 and 5.**
  - **Attribution.** The session's best roleplay beat — persuading Manshoon to move the duel — was credited to the wrong player by the GMAssist recap **and, independently, by Zoom's own summary.** Both were wrong the same way; the raw VTT speaker labels settled it in one grep. Nothing that matters about "who did what" survives a summary-only check.
  - **Retraction.** The GM audibly retracted 13 damage mid-sentence while both summaries recorded it as standing.
  - **Read-aloud.** The GM recapped the prior session by reading his own written summary out loud, and the ASR collapsed *"Thorin, suspicious of the GM's repeated questions about exact positions, was proved right"* into **"Alyss proved right"** — a name-shaped garble with no referent, one small step from being resolved to the nearest real NPC. **When the GM reads a written passage aloud, diff the quote against the source document, not against your ear.**
  - **Edition.** The audit cited "no damage on a made save" — **PHB 2014** — at a table running **PHB 2024**, where a made save deals **half damage and ends the spell**. Half of 26 is the 13 the GM had called. Two consecutive passes reversed a correct ruling on the strength of the wrong edition, while a player had quoted 2024 verbatim on tape the whole time (*"disadvantage on ability checks and attack rolls"*, *"he gets half the damage"*).
