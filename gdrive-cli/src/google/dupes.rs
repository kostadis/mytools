...
fn build_path_lookup(records: &[DriveRecord]) -> HashMap<String, String> {
    ...
    let folders: HashMap<String, FolderEntry> = records
        .iter()
        .filter(|r| r.mime_type == "application/vnd.google-apps.folder")
        .map(|r| {
            let fe = FolderEntry {
                id: r.id.clone(),
                name: r.name.clone(),
...
fn file_path(entry: &serde_json::Value, path_lookup: &HashMap<String, String>) -> String {
    let parents: Vec<String> = entry["parents"].as_array().map(|a| {
...
pub fn run(jsonl: &str, exclude: &[String], min_group: usize) -> Result<()> {
    ...
    for rec in &records {
        if rec.mime_type == "application/vnd.google-apps.folder" {
            continue;
        }
...
mod tests {
    ...
    fn test_load_jsonl_valid() {
        ...
        let line1 = r#"{"id":"1","name":"file1.txt","mime_type":"text/plain","parents":[],"size":100,"modified_time":"2024-01-01"}"#;
        let line2 = r#"{"id":"2","name":"file2.txt","mime_type":"text/plain","parents":["1"],"size":200,"modified_time":"2024-01-02"}"#;
